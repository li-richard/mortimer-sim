...
- **Where the tasks on offer land** — one row per tier with two measures on a shared scale: the solid bar is P(the best task on offer lands in this tier), which sums to 100%; the faint bar behind it is P(at least one offered task lands there), which doesn't. For the top tier the two are necessarily equal, since nothing outranks it. Below it, how much of an offer your modifier rules actually move up or down a tier.
